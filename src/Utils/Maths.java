									...
									loc.add(Math.cos(radialsPerStep * step)
											* radius, 0,
											Math.sin(radialsPerStep * step)
													* radius);
									if (randomColor) {
										ParticleEffect.valueOf(particle).display(loc, 0.0F, 0.0F, 0.0F,1.0F, 1);
									} else {
										ParticleEffect.valueOf(particle).display(loc, 0.0F, 0.0F, 0.0F, 0.0F, 1);
									}
									step++;

								}
							}, HypixelEffect.config.refreshtime,
							HypixelEffect.config.refreshtime).getTaskId();
	...
	public static void radarEffect(final Player p, final String particle,
		...
		if (!countdown_id.containsKey(p)) {
			...
			int i = Bukkit.getServer().getScheduler()
					.runTaskTimer(HypixelEffect.plugin, new Runnable() {
						float j = 0.0F;
						...
						public void run() {
							...
							for (int k = 0; k < 5F; k++) {
								...
								if (randomColor) {
								ParticleEffect.valueOf(particle).display(loc,
					...
					.runTaskTimer(HypixelEffect.plugin, new Runnable() {
						...
						public void run() {
							for (int k = 0; k < lineNumber; k++) {
								...
								l.add(v);
								if (randomColor) {
									ParticleEffect.valueOf(particle).display(
											l, 0, 0, 0, 1, 1);
								} else {
									ParticleEffect.valueOf(particle).display(
											l, 0, 0, 0, 0, 1);
	...
	public static void stopRotation(Player p) {
		if (countdown_id.containsKey(p)) {
			...
			if (HypixelEffect.config.specialSounds)
				...
				p.sendMessage(HypixelEffect.config.hypixeleffectAlreadyStop
						.replace("&", "§"));
